...
export class CheckListService {
  ...
  getList = (input: GetCheckListListDto, config?: Partial<Rest.Config>) =>
    this.restService.request<any, PagedResultDto<CheckListDto>>({
      method: 'GET',
      url: '/api/app/check-lists',
      params: { filter: input.filter, name: input.name, position: input.position, checkListType: input.checkListType, carModelId: input.carModelId, sorting: input.sorting, skipCount: input.skipCount, maxResultCount: input.maxResultCount },
    },
    { apiName: this.apiName,...config });
